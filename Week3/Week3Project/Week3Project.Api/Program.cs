...
// GET: Fetches a complete list of all cards tracked in the database
    app.MapGet("/Cards", async (StoreDbContext db) => { return await db.Cards.ToListAsync(); });
...
// GET: Analytics report showing the top 10 products sorted by total volume of units ordered
    app.MapGet("/reports/top-products", async (StoreDbContext db, CancellationToken ctk) =>
    {
        ...
        var topProducts = await db.OrderLines
            ...
            .Take(5) // Restrict to top 10 items to preserve network bandwidth
            .ToListAsync(ctk);
...
// POST: Idempotent database seeder that safely populates cards and target stock counts
...
// POST: Generates massive volume orders and routes them off to background processing threads asynchronously
        ...
        IHostApplicationLifetime appLifetime) =>
    {
        ...
        IReadOnlyList<int> ids = seeder.Seed(i);
        ...
        return Results.Accepted(value: new
            { Message = "Burst processing started in background.", OrderCount = ids.Count });
    });
...
// POST: Diagnostic utility comparing Sequential processing throughput vs Parallel processing pipelines
        ...
        StoreDbContext db, HttpContext httpContext) =>
    {
        ...
        IReadOnlyList<int> sampleOrderIds = seeder.Seed(n);
        ...
        await fulfillmentService.MicroPlasticBurstAsync(sampleOrderIds, ctk, false);
    ...
    // PATCH: Flushes all inventory counts back to zero
    app.MapPatch("/Clear", async (IDbContextFactory<StoreDbContext> dbContextFactory) =>
    {
...
// PATCH: SQL execution to restock all items back to 1000 items instantly
    app.MapPatch("/Restock", async (StoreDbContext db) =>
    {
        ...
        try
        {
            ...
            return Results.Ok(new
                { Message = "All card quantities successfully set to 1000.", RowsUpdated = affectedRows });
        }
        catch (Exception ex)
        {
