...
--COUNT() - 4 - COUNT(*) is different from count (Same_Column)
...
select UPPER(LastName) as LastUpper,
        len(Email) as Emailen,
        concat(FirstName, ' ', LastName) as FullName,
        datediff(DAY, JoinedDate, GETDATE()) as DayMember
from dbo.Member;
...
--Books with their caegories (FK = PK)
...
--GROUP BY + HAVING with JOIN
--I want the name, total books in, and total copies across all books in: A caegory
...
--LEFT JOIN - we want all records from the table, and matching records from the right
--every member and their loans if they have any, member with no loans wiill still appear
...
--every book and their loan if it exist
...
---FULL OUTER vs CROSS
--FULL OUT JOIN - returns mmatched rows whee they exist, as well as unmatched rows
select b.Title, c.Name as Category
from dbo.Book b
full outer join dbo.Category c on c.CategoryId = b.CategoryId
order by c.Name;

--CROSS JOIN - not common
--Every possible combination of the data of the rows in both tables
--very rare
...
--Use a join to combine ccolummns from multiple tables into the output
--Use a subquery to filter against a computer value or set you DONT need in the output
--usually a JOIN is easier to write
...
--correlated subquery: this is chaos sorcery
select Title, TotalCopies
from dbo.Book b1
where TotalCopies > (
...
join dbo.Category as c ON cast(c.CategoryId = b.CategoryId
